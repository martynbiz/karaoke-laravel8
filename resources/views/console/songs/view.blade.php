        ...
        <div class="small-12 cell">
            <nav aria-label="You are here:" role="navigation">
                <ul class="breadcrumbs">
                    ...
                    <li><a href="{{ route('console_artists_view', ['artist_id' => $song->artist->id]) }}">{{ $song->artist->name }}</a></li>
            ...
            <ul class="song-tag-list">
                <?php foreach ($song->tags()->where('is_valid', 1)->get() as $tag): ?>
                    <li><a href="{{ route('console_tags_view', ['tag_id' => $tag->id]) }}?back_path={{ route('console_songs_view', ['song_id' => $song->id]) }}" class="button">{{ $tag->name }}</a></li>
                <?php endforeach ?>
            </ul>
            <hr>
            <form action="{{ route('console_playlist_post', ['song_id' => $song->id]) }}" method="post" id="songs_queue_form">
				{{ csrf_field() }}
